...
class Database {
        ...
        `).then(() => {
            ...
            return this.enhancer.safeQuery('run', `
                CREATE TABLE IF NOT EXISTS ticket_type_roles (
                    ...
                    PRIMARY KEY (guild_id, ticket_type, role_id)
                )
            `);
        }).then(() => {
    ...
    async getAutoRoles(guildId, type) {
        const sql = `
            SELECT role_id FROM ticket_type_roles
            WHERE guild_id = ? AND ticket_type = ?
        `;
        return this.enhancer.safeQuery('all', sql, [guildId, type]).then(rows => {
            return (rows || []).map(row => row.role_id);
        });
    }

    async addAutoRole(guildId, type, roleId, addedBy) {
        const sql = `
            INSERT OR REPLACE INTO ticket_type_roles
            (guild_id, ticket_type, role_id, added_by, added_at)
            VALUES (?, ?, ?, ?, ?)
        `;
        return this.enhancer.safeQuery('run', sql, [guildId, type, roleId, addedBy, Date.now()]);
    }

    async removeAutoRole(guildId, type, roleId) {
        const sql = `
            DELETE FROM ticket_type_roles
            WHERE guild_id = ? AND ticket_type = ? AND role_id = ?
        `;
        return this.enhancer.safeQuery('run', sql, [guildId, type, roleId]);
    }

    async listAutoRoles(guildId) {
        const sql = `
            SELECT ticket_type, role_id, added_by, added_at
            FROM ticket_type_roles
            WHERE guild_id = ?
            ORDER BY ticket_type, added_at
        `;
        return this.enhancer.safeQuery('all', sql, [guildId]).then(rows => rows || []);
    }

    async getTicketsByTypeAndRange(guildId, type, startNumber, endNumber) {
        const sql = `
            SELECT * FROM tickets
            WHERE guild_id = ? AND type = ?
              AND CAST(name AS INTEGER) >= ?
              AND CAST(name AS INTEGER) <= ?
              AND status != 'deleted'
            ORDER BY CAST(name AS INTEGER) ASC
        `;
        return this.enhancer.safeQuery('all', sql, [guildId, type, startNumber, endNumber]).then(rows => {
            return (rows || []).map(row => ({
                id: row.id,
                guildId: row.guild_id,
                channelId: row.channel_id,
